<template>
  <div>
    ...
    <div class="all-img-box">
      <div :class="[!problem.img_query ? 'select-img' : '']" v-if="problem.img" @click="imgrefload">
  ...
  methods: {
    ...
    bindImg(idx) {
      ...
      if(this.imgIdx == idx){
        ...
        this.problem.img_list[idx].img = this.problem.img_list[idx].img +'?'+new Date()
        this.imgIdx = idx
        if(!this.problem.img_query){
          this.problem.img_query = true
        }
      }
    },
    getList() {
